namespace CodeStage.AntiCheat.Editor.Editors
{
	...
	internal class ActDetectorEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			{
				EditorGUILayout.LabelField(new GUIContent("You need to add at least one active item to the Detection Event in order to use Auto Start feature!"), ActEditorStyles.BoldWrappedLabel);
			}
			else if (!autoStart.boolValue)
			{
				EditorGUILayout.LabelField(new GUIContent("Don't forget to start detection!", "You should start detector from code using ObscuredCheatingDetector.StartDetection() method. See readme for details."), ActEditorStyles.BoldWrappedLabel);
				EditorGUILayout.Separator();
			}
